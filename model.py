...
def make_aligned_causal_cross_mask(T_dec: int, T_enc: int, device):
    dec_pos = torch.arange(T_dec, device=device).unsqueeze(1)
    enc_pos = torch.arange(T_enc, device=device).unsqueeze(0)
    m = (enc_pos <= (dec_pos - 1))
    if T_enc > 0:
        no_any_true = ~m.any(dim=1)
        m[no_any_true, 0] = True
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q_in, k_in, v_in, attn_mask=None):
        ...
        q = self.q_proj(q_in).view(B, Tq, self.n_head, self.d_head).transpose(1, 2)
        k = self.k_proj(k_in).view(B, Tk, self.n_head, self.d_head).transpose(1, 2)
        v = self.v_proj(v_in).view(B, Tk, self.n_head, self.d_head).transpose(1, 2)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask=None):
        x = x + self.mha(self.ln1(x), self.ln1(x), self.ln1(x), src_mask)
...
class TinyTransformerSeq2Seq(nn.Module):
    ...
    def forward(self, src_ids, tgt_in_ids):
        ...
        tgt_mask   = causal_mask(Tt, device)
        cross_mask = make_aligned_causal_cross_mask(Tt, Ts, device)
